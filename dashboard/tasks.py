...
def grade(event):
	...
	for result in results:
		testresult = TestCaseResult(submission=submission, test_case=problem_obj.test_case_group.testcase_set.filter(preliminary=event['preliminary'], num=result['test_case']).get(), result=result['status'], stdout=result['stdout'], stderr=result['stderr'])
		testresult.save()
